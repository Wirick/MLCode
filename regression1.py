...
# Seismic local linear regression function setup, takes a location and
# predicts the time residual using the K nearest neighbors on EXAMPLES,
# and using the seismic metric as the distance function.
def residual_regression_setup(initial, size, examples):
    return lambda x : klocalLinearRegression(initial[0], initial[1], 
												[float(x[6]), float(x[7])], 
												examples, size)

# Seismic error function between a residual prediction fx and the actual residual at x.
def residual_err(x, fx):
	return util.one_norm(float(x[10]), fx[0]) 

# function for converting the argument of the seismic metric into a hashable key.
def residual_hash(x):
	comp1 = tuple([tuple([str(x[0][0][0]), str(x[0][0][1])]), x[0][1], x[0][2]])
	comp2 = tuple([tuple([str(x[1][6]), str(x[1][7])]), x[1][9], x[1][5]])
	return comp1, comp2
